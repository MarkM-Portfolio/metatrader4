...
#property description "Author: Mark Mon Monteros"
#property copyright "Finwaze © 2023 All Rights Reserved."
#property icon "eh.ico"
#property strict
...
void NewOrder() {
   ...
   if (channelTradeAction != NULL && currencyTotalOrders == 0 && !stopTrade && isRSI && isRSIRef && isStochRSI && isStochRSIRef && isIchiMoku && isIchiMokuRef && isTechAnalysis && isTechAnalysisRef) {
      ...
      if (channelTradeAction == "buy") {
         int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slippage, buySL, buyTP, symbol + period + " GOLD_v9.6 " + timeStamp, MagicNumber, 0, orderColor);
      ...
      if (channelTradeAction == "sell") {
         int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slippage, sellSL, sellTP, symbol + period + " GOLD_v9.6 " + timeStamp, MagicNumber, 0, orderColor);
...
void TradeNews() {
   ...
   if (currencyTotalOrders == 0) {
      ...
      int ticketBuy = OrderSend(Symbol(), OP_BUY, VolumeSize, MarketInfo(Symbol(), MODE_ASK), Slippage, buySL, buyTP, symbol + period + " GB-NEWS_v9.6 " + timeStamp, MagicNumber, 0, clrBlue);
      ...
      int ticketSell = OrderSend(Symbol(), OP_SELL, VolumeSize, MarketInfo(Symbol(), MODE_BID), Slippage, sellSL, sellTP, symbol + period + " GB-NEWS_v9.6 " + timeStamp, MagicNumber, 0, clrRed);
