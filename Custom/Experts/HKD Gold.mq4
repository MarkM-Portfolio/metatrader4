...
// Guide
// ConstantLot 0.1 == TakeProfit 370 == LimitSize 0.5
// ConstantLot 0.05 == TakeProfit 30 to 70 == LimitSize 0.03
// ConstantLot 0.01 == TakeProfit 30 to 70 == LimitSize 0.02

extern double ConstantLot = 0.1;
extern double TakeProfit = 370;
extern double Step = 70;
extern double LimitSize = 0.52;
extern double Multiplicator = 1.6;
extern int MaxOrders = 4;
extern double Percentage_Of_Autolot_Volume = 30;
extern bool TrendConfirm = true;
extern bool NoSL = true;
extern int SLPoints = 300;
extern string NoTPMode = "use optimized trailing stop";
extern bool NoTP = false;
extern bool ForceTP = false;
extern double ForceTPValue = 1.30;
...
extern string Riskmanagment = "Closing all positions when the specified level is reached. (Close Set)";
extern bool Use_Restrictions = false;
extern int Slippage = 0;
extern int Magic = 3579;
extern string Logo = "Demonstration of the logo";
extern bool ShowLogo = false;
...
int init() {
   ...
   if (Gi_0000 == 1) { 
      ...
      tmp_str0001 = "            HKD Gold  v" + VERSION + " " + symbol_str;
      tmp_str0001 = tmp_str0001 + "  Time Frame: ";
      tmp_str0002 = period_str;
      tmp_str0001 = tmp_str0001 + tmp_str0002;
      Comment(tmp_str0001, "\n", "            Server: ", tmp_str0000, "\n", "            Order Time:  ", ToUpper(TimeTo12HourFormat(timeStamp, false)), "\n", "            Lots:  ", Id_0038, "\n", "            Symbol: ", symbol_str, "\n", "            Price:  ", NormalizeDouble(Bid, 4), "\n");
   } 

   Gi_0000 = IsTesting();
   if (Gi_0000 == 0) { 
      tmp_str0002 = ServerAddress();
      tmp_str0003 = "            HKD Gold v" + VERSION + " " + symbol_str;
      tmp_str0003 = tmp_str0003 + "  Time Frame: ";
      tmp_str0004 = period_str;
...
void OnTick() {
   ...
   if ((IsOptimization() == false && IsTesting() == false && !IsVisualMode()) || (ShowLogo && IsTesting() && IsOptimization() == false)) {      
      f0_13();
      // f0_10();
   }
   ...
   if (Ib_0078 == 0 || (Ib_0078 && Ii_0080 > Ii_007C && Hour() >= Ii_007C && Hour() <= Ii_0080) || (Ii_007C > Ii_0080 && !Gb_003E)) {   
      Ls_FFA0 = "true";
   }
   ...
   if ((Ib_0078 && Ii_0080 > Ii_007C && Gb_003E == false) || (Ii_007C > Ii_0080 && Hour() >= Ii_0080 && Hour() <= Ii_007C)) {
      Ls_FF90 = "true";
   }
   ...
   if (isNewTick()) {
      ...
      if (NoTP) {
         ...
         if (currencyTotalOrders != 0) CheckOpenOrders();
      }
   }
}
...
int perform_order_op(int Fa_i_00, double Fa_d_01, double Fa_d_02, int Fa_i_03, double Fa_d_04, double Fa_d_05, int Fa_i_06, string Fa_s_07, int Fa_i_08, int Fa_i_09, int Fa_i_0A) {
   Fa_s_07 = TimeTo12HourFormat(timeStamp, false) + " " + symbol_str + "(" + period_str + ") HKD Gold " + VERSION; // Order comment override

   if (TrendConfirm) isMegaTrend = false;
   ...
   if (returned_i == 2){
      ...
      do { 
         ...
         // Set StopLoss
         if (!NoSL) {
            if (Fa_i_00 == 0) Gd_0000 = Fa_d_02 - (_Point * SLPoints); //buy
            if (Fa_i_00 == 1) Gd_0000 = Fa_d_02 + (_Point * SLPoints); //sell
         }
         ...
         if (TrendConfirm) {
            ...
            // kijunRSICheck = KijunRSICheck();
            
            // if (Fa_i_00 == 0 && megaTrendCheck == "buy" && kijunRSICheck < -25) {
            if (Fa_i_00 == 0 && megaTrendCheck == "buy") {
   ...
   if (returned_i == 4){
      ...
      do { 
         ...
         // Set StopLoss
         if (!NoSL) {
            if (Fa_i_00 == 0) Gd_0006 = Fa_d_02 - (_Point * SLPoints); //buy
            if (Fa_i_00 == 1) Gd_0006 = Fa_d_02 + (_Point * SLPoints); //sell
         }
   ...
   if (returned_i == 0) {
      ...
      do { 
         ...
         // Set StopLoss
         if (!NoSL) {
            if (Fa_i_00 == 0) Gd_000C = Fa_d_02 - (_Point * SLPoints); //buy
            if (Fa_i_00 == 1) Gd_000C = Fa_d_02 + (_Point * SLPoints); //sell
         }
   ...
   if (returned_i == 3) {
      ...
      do { 
         ...
         // Set StopLoss
         if (!NoSL) {
            if (Fa_i_00 == 0) Gd_0012 = Fa_d_02 - (_Point * SLPoints); //buy
            if (Fa_i_00 == 1) Gd_0012 = Fa_d_02 + (_Point * SLPoints); //sell
         }
         ...
         if (TrendConfirm) {
            ...
            // if (Fa_i_00 == 1 && megaTrendCheck == "sell" && kijunRSICheck > 25) {
            if (Fa_i_00 == 1 && megaTrendCheck == "sell") {
   ...
   if (returned_i == 5) {
      ...
      do { 
         ...
         // Set StopLoss
         if (!NoSL) {
            if (Fa_i_00 == 0) Gd_0018 = Fa_d_02 - (_Point * SLPoints); //buy
            if (Fa_i_00 == 1) Gd_0018 = Fa_d_02 + (_Point * SLPoints); //sell
         }
   ...
   if (returned_i == 1){
      ...
      do { 
         ...
         // Set StopLoss
         if (!NoSL) {
            if (Fa_i_00 == 0) Gd_001E = Fa_d_02 - (_Point * SLPoints); //buy
            if (Fa_i_00 == 1) Gd_001E = Fa_d_02 + (_Point * SLPoints); //sell
         }
...
void f0_13() {
   ...
   if (ObjectFind(Ls_FFE8) == -1) { 
      ...
      ObjectSet(Ls_FFE8, OBJPROP_YDISTANCE, 36);
   }

   tmp_str0000 = "Profit Today: " + DoubleToString(Ld_FFF8, 2);
   ObjectSetText(Ls_FFE8, tmp_str0000, Ii_0108, "Tahoma", 7798721);
   Gi_0003 = 1;
   Gd_0004 = 0;
   Gi_0005 = 0;
   ...
   if (ObjectFind(Ls_FFE8) == -1) { 
      ...
      ObjectSet(Ls_FFE8, OBJPROP_CORNER, 1);
      ObjectSet(Ls_FFE8, OBJPROP_XDISTANCE, 10);
      ObjectSet(Ls_FFE8, OBJPROP_YDISTANCE, 54);
   } 

   tmp_str0001 = "Profit Yesterday: " + DoubleToString(Ld_FFF8, 2);
   ObjectSetText(Ls_FFE8, tmp_str0001, Ii_0108, "Tahoma", 7798721);
   Gi_0006 = 2;
   Gd_0007 = 0;
   Gi_0008 = 0;
   ...
   if (ObjectFind(Ls_FFE8) == -1) { 
      ...
      ObjectSet(Ls_FFE8, OBJPROP_CORNER, 1);
      ObjectSet(Ls_FFE8, OBJPROP_XDISTANCE, 10);
      ObjectSet(Ls_FFE8, OBJPROP_YDISTANCE, 72);
   } 

   tmp_str0002 = "Balance: " + DoubleToString(AccountBalance(), 2);
   ObjectSetText(Ls_FFE8, tmp_str0002, Ii_0108, "Arial", Magenta);
   ...
   ObjectSetText(Ls_FFE8, Is_0018, Ii_0108, "Arial Black", Gold);
}
...
//    ObjectSetText(Ls_FFF0, "HKD Gold v" + VERSION, 28, "ARIAL BLACK", 9118347);
//    Ls_FFF0 = Is_0018 + "L_2";
...
void CheckOpenOrders() {
   ...
   for (int i=0; i<OrdersTotal(); i++) { 
      if(OrderSelect( i, SELECT_BY_POS, MODE_TRADES )) {
         if(OrderSymbol() == Symbol() && OrderMagicNumber() == Ii_0084) {
            getProfit = NormalizeDouble(OrderLots(), 2) * factor;
            ...
            if (currencyProfit > getProfit) {
               if (ForceTP) {
                  // force take profit
                  if ((currencyProfit >= getProfit * ForceTPValue)) {
   ...
   for (int i=0; i<OrdersTotal(); i++) { 
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
         if (OrderType() == OP_BUY) { // buy orders
            buyCounter++;
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == Ii_0084) currencyBuyCounter++;
         }
         if (OrderType() == OP_SELL) { // sell orders
            sellCounter++;
            if (OrderSymbol() == Symbol() && OrderMagicNumber() == Ii_0084) currencySellCounter++;
         }
      } 
   }
