...
input bool TechAnalysisEnable = true;
input bool NewsCloseTrade = true;
input bool NewsTradeOn = true;
input bool MondayDisableTrade = true;
input bool CloseSetEnable = true;
input int OrderLimit = 2;
...
void OnTick() {
   ...
   if (MondayDisableTrade) { // disable trade on Mondays
   ...
   if (isNewTick()) {
      ...
      if (currencyTotalOrders != 0) {
         if (NewsCloseTrade) {
            CheckNews();
            if (NewsTradeOn && isNewsOn == "ONLINE") {
               TradeNews();
            }
         } else {
            isNewsOn = NULL;
...
bool TechnicalAnalysisCheckClose() {
   string techAnalysisAction = NULL;
   ...
   isTechAnalysis2 = false;

   if (channelTradeAction == techAnalysisAction) {
      isTechAnalysis2 = true;
   } else {
      isTechAnalysis2 = false;
   }

   return isTechAnalysis2;
}
...
void CheckOpenOrders() {
   ...
   closeSet = (VolumeSize * StrToInteger(initFactor)) * 2;
         ...
         if(OrderSymbol() == Symbol()) {
            if (OrderType() == OP_BUY) {
               ...
               if (TechAnalysisEnable) {
                  isTechAnalysis2 = TechnicalAnalysisCheckClose();
               } else {
                  isTechAnalysis2 = true;
               }
               ...
               //    if ((black + 3) < red || !isTechAnalysis2) {
   ...
   Comment("Stop Trade (News): ", stopTrade, " Reversal Order: ", isReversalOrder, " isStochOverCheck: ", isStochOverCheck, " isStochOver: ", isStochOver, " isOverTrade: ", isOverTrade, " isCloseSet: ", (CloseSetEnable) ? closeSetMode : ToUpper("Disabled"), " || INIT FACTOR: ", initFactor, " FACTOR: ", factor, " BORDER FACTOR: ", borderFactor, " GETPROFIT: ", getProfit, " TAKEPROFIT: ", takeProfit, " CURRENCYPROFIT: ", currencyProfit, " BUY LIMIT COUNT: ", IntegerToString(buyLimitCounter) + "/" + IntegerToString(OrderLimit), " SELL LIMIT COUNT: ", IntegerToString(sellLimitCounter) + "/" + IntegerToString(OrderLimit));
}
...
void NewOrder() {
    ...
    if (channelTradeAction != NULL && currencyTotalOrders == 0 && !stopTrade && isRSI && isStochRSI && isTechAnalysis) {
        ...
        if (channelTradeAction == "buy") {
            int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slippage, buySL, buyTP, symbol + period + " VISTA_v8.2 " + timeStamp, MagicNumber, 0, orderColor);
        ...
        if (channelTradeAction == "sell") {
            int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slippage, sellSL, sellTP, symbol + period + " VISTA_v8.2 " + timeStamp, MagicNumber, 0, orderColor);
...
void TradeNews() {
   ...
   if (currencyTotalOrders == 0) {
      ...
      if (!isRSI || !isStochRSI) { // stop news trading when condition not met after 5mins
         if (IsNewCandle()) {
            isReversalOrder = true;
            isStochOverCheck = false;
      ...
      for (int i=0; i<OrdersTotal(); i++ ) { 
         if (OrderSelect( i, SELECT_BY_POS, MODE_TRADES )) {
            if (OrderSymbol() == Symbol()) {
               ...
               if (OrderType() == OP_BUY) {
                  Print("Trend Changed Due to News.. Now Closing!!");
