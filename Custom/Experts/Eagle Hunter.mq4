...
void NewOrder() {
   ...
   if (channelTradeAction != NULL && currencyTotalOrders == 0 && !stopTrade && isRSI && isRSIRef && isStochRSI && isStochRSIRef && isIchiMoku && isIchiMokuRef && isTechAnalysis && isTechAnalysisRef) {
      ...
      if (channelTradeAction == "buy") {
         int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slippage, buySL, buyTP, symbol + period + " EAGLE_v9.6 " + timeStamp, MagicNumber, 0, orderColor);
      ...
      if (channelTradeAction == "sell") {
         int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slippage, sellSL, sellTP, symbol + period + " EAGLE_v9.6 " + timeStamp, MagicNumber, 0, orderColor);
