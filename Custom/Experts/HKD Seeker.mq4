...
//     }
    ...
    //     return channelTradeAction;
    double black_rsi_1 = iCustom(Symbol(), 1, "\\Custom\\FINWAZE RSI CROSSOVER BASIC", 14, 14, 0.618, 1, 0); //RSI 14
    double red_rsi_1 = iCustom(Symbol(), 1, "\\Custom\\FINWAZE RSI CROSSOVER BASIC", 14, 14, 0.618, 0, 0); //finwaze 14  
    double black_stochrsi_1 = iCustom(Symbol(), 1, "\\Custom\\FINWAZE STOCHRSI VER 3", 3, 3, 14, 14, PRICE_CLOSE, 0, 0); //K
    double red_stochrsi_1 = iCustom(Symbol(), 1, "\\Custom\\FINWAZE STOCHRSI VER 3", 3, 3, 14, 14, PRICE_CLOSE, 1, 0); //D

    double black_rsi_5 = iCustom(Symbol(), 5, "\\Custom\\FINWAZE RSI CROSSOVER BASIC", 14, 14, 0.618, 1, 0); //RSI 14
    double red_rsi_5 = iCustom(Symbol(), 5, "\\Custom\\FINWAZE RSI CROSSOVER BASIC", 14, 14, 0.618, 0, 0); //finwaze 14  
    double black_stochrsi_5 = iCustom(Symbol(), 5, "\\Custom\\FINWAZE STOCHRSI VER 3", 3, 3, 14, 14, PRICE_CLOSE, 0, 0); //K
    double red_stochrsi_5 = iCustom(Symbol(), 5, "\\Custom\\FINWAZE STOCHRSI VER 3", 3, 3, 14, 14, PRICE_CLOSE, 1, 0); //D

    double black_rsi_15 = iCustom(Symbol(), 15, "\\Custom\\FINWAZE RSI CROSSOVER BASIC", 14, 14, 0.618, 1, 0); //RSI 14
    double red_rsi_15 = iCustom(Symbol(), 15, "\\Custom\\FINWAZE RSI CROSSOVER BASIC", 14, 14, 0.618, 0, 0); //finwaze 14  
    double black_stochrsi_15 = iCustom(Symbol(), 15, "\\Custom\\FINWAZE STOCHRSI VER 3", 3, 3, 14, 14, PRICE_CLOSE, 0, 0); //K
    double red_stochrsi_15 = iCustom(Symbol(), 15, "\\Custom\\FINWAZE STOCHRSI VER 3", 3, 3, 14, 14, PRICE_CLOSE, 1, 0); //D

    channelTradeAction = NULL;

    if ((black_rsi_1 - 1) > red_rsi_1 && (black_stochrsi_1 - 1) > red_stochrsi_1 && (black_rsi_5 - 1) > red_rsi_5 && (black_stochrsi_5 - 1) > red_stochrsi_5 && (black_rsi_15) > red_rsi_15 && (black_stochrsi_15) > red_stochrsi_15) {
...
void PrepareOrder() {
   ...
   if (channelTradeAction != NULL && currencyTotalOrders == 0 && !stopTrade) {
      ...
      string comment = TimeTo12HourFormat(timeStamp, false) + " " + symbol_str + "(" + period_str + ") HKD Seeker " + VERSION;
      string printOut;
...
void NewOrder(string printOut, int orderType, double openPrice, color orderColor, double sL, double tP, string comment) {
   if (!newsOrder) {
      ...
      comment = "NEWS >> " + TimeTo12HourFormat(timeStamp, false) + ") HKD Seeker " + VERSION;
