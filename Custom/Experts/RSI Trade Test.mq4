...
void OnTick() {
   ...
   if (isNewTick()) {
    ...
    //   if (currencyTotalOrders == 0 && !stopTrade && prevBlackRSI != NULL && prevRedRSI != NULL && prevBlackStochRSI != NULL && prevRedStochRSI != NULL && buyLimitCounter <= 3 && sellLimitCounter <= 3) {
      if (currencyTotalOrders == 0 && !stopTrade && prevBlackRSI != NULL && prevRedRSI != NULL && prevBlackStochRSI != NULL && prevRedStochRSI != NULL) {
         PrepareOrder();
         NewOrder();
      }
   }
   ...
   for (int i=0; i<OrdersTotal(); i++) { 
      if(OrderSelect( i, SELECT_BY_POS, MODE_TRADES )) {
         if(OrderSymbol() == Symbol()) {
            ...
            if (currencyProfit > getProfit) {
               ...
               if (currencyProfit <= StringToDouble(takeProfit) && takeProfit != NULL) {
                  ...
                  if (currencyProfit >= StrToInteger(borderFactor)) {
                     if (OrderType() == OP_BUY && barClose > barOpen && (barClose + 0.001 < barHigh) && black < red) {
                        Print("TAKE PROFIT NOW +-----> ", currencyProfit);
                        FileDelete(filename0);
                        FileDelete(filename1);
                        FileDelete(filename2);
                     ...
                     if (OrderType() == OP_SELL && barClose < barOpen && (barClose - 0.001 > barLow) && black > red) {
                        Print("TAKE PROFIT NOW +-----> ", currencyProfit);
                        FileDelete(filename0);
                        FileDelete(filename1);
                        FileDelete(filename2);
            ...
            } else if ((StrToInteger(initFactor) >= 160) && currencyProfit < getProfit && (currencyProfit >= getProfit * 0.90) && takeProfit == NULL) { // almost reached 90% profit
               // Close order if can't reach threshold
               if (OrderType() == OP_BUY && barClose > barOpen && (barClose + 0.001 < barHigh) && black < red) {
                  factor -= 1;

                  Print("Can't Reach Factor. SET NEW FACTOR...", factor);
                  int filehandle2_w = FileOpen(filename2, FILE_WRITE|FILE_TXT);
                  FileWriteString(filehandle2_w, IntegerToString(factor));
                  FileClose(filehandle2_w);
...
void NewOrder() {
   ...
   if (channelTradeAction != NULL && currencyTotalOrders == 0 && isRSI && isStochRSI) {
      ...
      if (channelTradeAction == "buy") {
         int ticket = OrderSend(Symbol(), orderType, VolumeSize, openPrice, Slipage, 0, 0, symbol + period + " RSITEST_v7.3 " + timeStamp, MagicNumber, 0, orderColor);
